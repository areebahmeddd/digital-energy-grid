...
**TPDDL** (Tata Power Delhi Distribution, the utility) publishes a demand-flex **procurement schedule** as an OpenADR-aligned time series — one interval per tranche, each carrying a `CAPACITY_REQUESTED` (kW), a `PRICE` (INR/kWh; a negative price pays for extra consumption) and a `SHORTFALL_PENALTY`. This is the monopsony end of a market: a single buyer fixing the clearing price for any quantity (uc2 is the multi-party pay-as-clear auction of the same model). **GreenFlex Aggregator** discovers the opportunity, enrolls participating grid meters (each with EnergyResources behind it — EV chargers here), and adds its per-slot `CAPACITY_OFFERED` column on the commitment. After the event, TPDDL publishes per-meter `BASELINE`/`USAGE` telemetry on the shared interval grid; settlement runs **per interval, per meter** and sums (e.g. two 30-min slots → 436.25 INR).

**Settlement is utility-only.** Revenue is computed against the DISCOM's own grid-meter measurements (`BASELINE` and `USAGE` per interval), authored by the utility (BPP) and pushed via `on_status`. The settlement rego ([`demand-flex-contractpolicy.rego`](../../specification/policies/demand-flex-contractpolicy.rego)) is hard-wired to ignore any performance record whose `methodology` is in the non-settlement allowlist (`RESOURCE_TELEMETRY`) — even if such a record is the first one in the payload, even if it is the only record on the wire (in which case the rego refuses to settle and surfaces an explicit violation). Baselines do not appear in EnergyResource telemetry at all.
...
|--------|------|-------------|
| [DemandFlexNeed](../../specification/schema/DemandFlexNeed/v2.0/) | `resourceAttributes` | Unified OpenADR TimeSeries procurement schedule (`intervalPeriod` + `intervals`); columns open in schema, rego-locked. uc1: `CAPACITY_REQUESTED` / `PRICE` / `SHORTFALL_PENALTY` per slot; location |
| [DemandFlexBuyOffer](../../specification/schema/DemandFlexBuyOffer/v2.0/) | `offerAttributes` | Buyer scalar inputs (`currency`, `baselineMethodology`); seller `participatingMeters` / `energyResources` / `reportDescriptors`. Per-slot `CAPACITY_OFFERED` rides `commitmentAttributes` |
...
The cross-field type-coverage and cardinality checks in [`demand_flex_networkpolicy.rego`](./policies/demand_flex_networkpolicy.rego) operate per-meter — they fire on each page in isolation, with no awareness of the wider delivery. That's fine: any malformed meter telemetry NACKs immediately on the page that carries it, regardless of the page's position. Settlement runs against the assembled view only (see [`demand-flex-contractpolicy.rego`](../../specification/policies/demand-flex-contractpolicy.rego)).
...
A single rego file ([`policies/demand_flex_networkpolicy.rego`](./policies/demand_flex_networkpolicy.rego), mirrors [`specification/policies/demand-flex-networkpolicy.rego`](../../specification/policies/demand-flex-networkpolicy.rego)) backs every networkId with one `violations` rule that enforces:
...
Settlement rego is referenced per-contract via `contractAttributes.policy.url` → [`specification/policies/demand-flex-contractpolicy.rego`](../../specification/policies/demand-flex-contractpolicy.rego), package `deg.contracts.demand_flex`. It picks the first `performance[*]` record whose `performanceAttributes.methodology` is NOT in the non-settlement allowlist (`{"RESOURCE_TELEMETRY"}`) and computes a net-zero `buyer pays / seller receives` revenue flow from its per-meter `BASELINE` / `USAGE`. If a payload carries only RESOURCE_TELEMETRY perf records, the rego emits an explicit `"no settlement-eligible performance record found"` violation rather than coercing resource numbers into the settlement — see [`demand-flex-contractpolicy_test.rego`](../../specification/policies/test/demand-flex-contractpolicy_test.rego) `test_resource_perf_record_excluded_from_settlement` and `test_resource_only_payload_violation`.
