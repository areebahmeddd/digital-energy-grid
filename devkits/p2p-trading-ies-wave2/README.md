...
- **Network policy** (`opapolicychecker`) — declared in [`config/opa-network-policies.yaml`](./config/opa-network-policies.yaml) and loaded from [`specification/policies/p2p-trading-ies-wave2-networkpolicy.rego`](../../specification/policies/p2p-trading-ies-wave2-networkpolicy.rego), the single source of truth (docker-compose mounts `specification/policies/` into each container as `/app/policies`). Both BAP and BPP use a single `default:` entry — every message evaluates against the same rules regardless of `context.networkId`. Add per-networkId entries as the network matures.
- **Seller-discom (contract) policy** (`contractpolicyenforcer`) — every payload's `contractAttributes.policy.url` points to the DeDi public-dataset record `https://api.dedi.global/dedi/lookup/indiaenergystack.in/ies-policies/ies-p2p-network-settlement-rego-policy-v1`, whose `data_url` serves [`specification/policies/p2p-trading-ies-wave2-contractpolicy.rego`](../../specification/policies/p2p-trading-ies-wave2-contractpolicy.rego) — the policy of the *seller's* discom, linked by the catalog publisher into every trade involving that discom's prosumers (see the [discom policy guide](../../specification/policies/discom-policy-guide/) for authoring and publishing your own). The `contractpolicyenforcer` step (see [`config/local-p2p-trading-sellerapp.yaml`](./config/local-p2p-trading-sellerapp.yaml)) resolves the record — it accepts only URLs under the configured `allowedPolicyUrlPrefixes` (India Energy Stack's DeDi namespace), verifies `data_url_checksum` when the record carries one, retries transient fetch failures, and caches the compiled rego for `cacheTTL` (≥ 1 day) so DeDi sees at most one lookup per day. It runs in three modes on the sellerapp:
  - **Enforcement, trades** (`bppTxnReceiver`, `violationActions: "select,init,confirm"`) — the policy's `violations` set gates the trade: a buyer whose discom (`participants[role=buyerPlatform].participantAttributes.utilityId`) is outside the policy's `allowed_buyer_discoms` gets a synchronous **400 NACK** and the contract never forms. Enforcement is fail-closed — a missing or unfetchable policy also NACKs. Exercised by the `init-blocked-by-policy` arazzo workflow.
  - **Enforcement, publish** (`bppTxnCaller`, `violationActions: "publish"`, matching the compound `catalog/publish`) — seller-side self-protection: each catalog offer's linked policy (`offers[].offerAttributes.contractAttributes.policy`) is evaluated for applicability (`applicable_seller_discoms`), currency, and network membership, and a violating catalog is NACKed before it reaches the fabric.
  - **Injection** (`bppTxnCaller`, `on_status`, `violationActions: ""`) — once settled intervals exist, the rego computes the four-role `revenueFlows` array with the discom's charges itemized (buyer/seller-side wheeling and delivery-shortfall penalty rates default to `0` INR/kWh; platform charge cap `0.42` INR/kWh disclosed in the descriptions — each discom sets its own rates in its published policy); the result lands at `message.contract.consideration[id=auto-settlement-flows].considerationAttributes` (RevenueFlow JSON-LD, Beckn-native).

The devkit's default trading network is `indiaenergystack.in/test-ies-p2p-trading-network` (payloads, postman `{{domain}}` variable, sandbox fixture mounts); `nfh.global/testnet-deg` remains recognized everywhere as a secondary test network. Signature/registry lookups accept both via the `allowedNetworkIDs` key on the `dediregistry` plugin. SubscriberIds name the participating entity (not the protocol role): `buyerapp.example.com`, `sellerapp.example.com`, `seller-discom-ledger.example.com`, `buyer-discom-ledger.example.com`. All four are registered on both networks; signing keys are wired via [`config/local-p2p-trading-{buyerapp,sellerapp,ledger-{seller,buyer}discom}.yaml`](./config/).
