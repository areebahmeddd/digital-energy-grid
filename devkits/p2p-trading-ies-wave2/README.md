...
- **Contract policy** — every payload's `contractAttributes.policy.url` points to [`specification/policies/p2p_trading_revenue.rego`](../../specification/policies/p2p_trading_revenue.rego). On `on_status` the BPP-side `revenueflows` middleware fetches and evaluates that rego against the contract payload, then writes the four-role `revenueFlows` array back into `contractAttributes`. Wheeling and penalty charges are `0` placeholders in the rego today — flip them to real expressions when the tariff/penalty rules land; no payload changes needed.
