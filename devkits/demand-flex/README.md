...
**Settlement is utility-only.** Revenue is computed against the DISCOM's own grid-meter measurements (`BASELINE` and `USAGE` per interval), authored by the utility (BPP) and pushed via `on_status`. The settlement rego ([`demand_flex_contractpolicy.rego`](../../specification/policies/demand_flex_contractpolicy.rego)) is hard-wired to ignore any performance record whose `methodology` is in the non-settlement allowlist (`RESOURCE_TELEMETRY`) — even if such a record is the first one in the payload, even if it is the only record on the wire (in which case the rego refuses to settle and surfaces an explicit violation). Baselines do not appear in EnergyResource telemetry at all.
...
A single rego file ([`policies/demand_flex_networkpolicy.rego`](./policies/demand_flex_networkpolicy.rego), mirrors [`specification/policies/demand_flex_networkpolicy.rego`](../../specification/policies/demand_flex_networkpolicy.rego)) backs every networkId with one `violations` rule that enforces:
...
Settlement rego is referenced per-contract via `contractAttributes.policy.url` → [`specification/policies/demand_flex_contractpolicy.rego`](../../specification/policies/demand_flex_contractpolicy.rego), package `deg.contracts.demand_flex`. It picks the first `performance[*]` record whose `performanceAttributes.methodology` is NOT in the non-settlement allowlist (`{"RESOURCE_TELEMETRY"}`) and computes a net-zero `buyer pays / seller receives` revenue flow from its per-meter `BASELINE` / `USAGE`. If a payload carries only RESOURCE_TELEMETRY perf records, the rego emits an explicit `"no settlement-eligible performance record found"` violation rather than coercing resource numbers into the settlement — see [`demand_flex_contractpolicy_test.rego`](../../specification/policies/test/demand_flex_contractpolicy_test.rego) `test_resource_perf_record_excluded_from_settlement` and `test_resource_only_payload_violation`.
