#!/usr/bin/env bash
# Run the uc1-demand-flex Arazzo workflows via Redocly Respect.
#
...
#
# Usage (from uc1-demand-flex/workflows/):
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/demand-flex-arazzo-XXXXXX")"
trap 'rm -rf "$WORK"' EXIT
mkdir -p "$WORK/workflows" "$WORK/examples"

cp "$USECASE_ROOT/workflows/demand-flex.arazzo.yaml" "$WORK/workflows/"
...
if [ "$PUBLIC_URL" = "http://beckn-router:9000" ]; then
  exec npx --yes @redocly/cli respect \
    "$WORK/workflows/demand-flex.arazzo.yaml" \
    "${RESPECT_ARGS[@]}"
else
  exec npx --yes @redocly/cli respect \
    "$WORK/workflows/demand-flex.arazzo.yaml" \
