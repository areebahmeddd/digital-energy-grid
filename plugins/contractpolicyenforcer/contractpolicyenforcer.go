// Package contractpolicyenforcer is an onix plugin that computes settlement flows
...
// ContractPolicyEnforcer is a Step plugin that computes and injects settlement flows.
type ContractPolicyEnforcer struct {
	config *Config
	cache  *PolicyCache
}

// New creates a new ContractPolicyEnforcer plugin instance.
func New(cfg map[string]string) (*ContractPolicyEnforcer, error) {
	config, err := ParseConfig(cfg)
	if err != nil {
		return nil, fmt.Errorf("contractpolicyenforcer: config: %w", err)
	}

	cache := NewPolicyCache(config)

	fmt.Printf("[ContractPolicyEnforcer] Enabled=%v, actions=%v, cacheTTL=%s\n",
		config.Enabled, config.Actions, config.CacheTTL)

	return &ContractPolicyEnforcer{config: config, cache: cache}, nil
}

// Run implements the Step interface.
func (rf *ContractPolicyEnforcer) Run(ctx *model.StepContext) error {
	...
	if len(refs) == 0 {
		if enforced {
			return model.NewBadReqErr(fmt.Errorf(
				"contractpolicyenforcer: action %q requires a policy reference (contractAttributes.policy or per-offer offerAttributes.contractAttributes.policy)", action))
		}
		if rf.config.DebugLogging {
			log.Debug(ctx, "ContractPolicyEnforcer: no policy reference in message, skipping")
		}
		return nil
	}
	...
	if err := json.Unmarshal(ctx.Body, &input); err != nil {
		if enforced {
			return model.NewBadReqErr(fmt.Errorf("contractpolicyenforcer: failed to parse message body: %w", err))
		}
		log.Warnf(ctx, "ContractPolicyEnforcer: failed to parse message body: %v", err)
		return nil
	}
	...
	for _, ref := range refs {
		...
		if !rf.config.IsPolicyURLAllowed(ref.URL) {
			if enforced {
				return model.NewBadReqErr(fmt.Errorf(
					"contractpolicyenforcer: policy URL does not match allowedPolicyUrlPrefixes: %s", ref.URL))
			}
			log.Warnf(ctx, "ContractPolicyEnforcer: policy URL does not match allowedPolicyUrlPrefixes: %s", ref.URL)
			continue
		}

		if rf.config.DebugLogging {
			log.Debugf(ctx, "ContractPolicyEnforcer: evaluating %s with query %s", ref.URL, ref.QueryPath)
		}

		// Get or compile the policy. On enforced actions this must be a
		// typed error so the pipeline reports the cause — e.g. a checksum
		// mismatch against the DeDi record — instead of a generic internal
		// error.
		pq, err := rf.cache.GetOrCompile(context.Background(), ref.URL, ref.QueryPath)
		if err != nil {
			if enforced {
				return model.NewBadReqErr(fmt.Errorf(
					"contractpolicyenforcer: policy %s could not be loaded (fail-closed on %s): %w", ref.URL, action, err))
			}
			log.Warnf(ctx, "ContractPolicyEnforcer: failed to load policy: %v", err)
		...
		if err != nil {
			if enforced {
				return model.NewBadReqErr(fmt.Errorf(
					"contractpolicyenforcer: rego evaluation of %s failed (fail-closed on %s): %w", ref.URL, action, err))
			}
			log.Warnf(ctx, "ContractPolicyEnforcer: rego evaluation failed: %v", err)
			continue // soft failure
		}
	...
	if len(violations) > 0 {
		if enforced {
			log.Warnf(ctx, "ContractPolicyEnforcer: NACKing %s — %d policy violation(s): %s",
	...
	if flows == nil {
		if rf.config.DebugLogging {
			log.Debug(ctx, "ContractPolicyEnforcer: no revenue_flows in rego result, skipping injection")
		}
		return nil
	}
	...
	if err != nil {
		log.Warnf(ctx, "ContractPolicyEnforcer: failed to inject revenue_flows: %v", err)
		return nil // soft failure
	}

	ctx.Body = modified
	log.Infof(ctx, "ContractPolicyEnforcer: injected %d settlement flow(s)", len(flows))
	return nil
}

// Close is a no-op cleanup function.
func (rf *ContractPolicyEnforcer) Close() {}
