...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ev-charging-arazzo-XXXXXX")"
trap 'rm -rf "$WORK"' EXIT
mkdir -p "$WORK/workflows" "$WORK/examples"

cp "$USECASE_ROOT/workflows/ev-charging.arazzo.yaml" "$WORK/workflows/"
...
if [ "$PUBLIC_URL" = "http://beckn-router:9000" ]; then
  exec npx --yes @redocly/cli respect \
    "$WORK/workflows/ev-charging.arazzo.yaml" \
    "${RESPECT_ARGS[@]}"
else
  exec npx --yes @redocly/cli respect \
    "$WORK/workflows/ev-charging.arazzo.yaml" \
