...
**Settlement is utility-only.** Revenue is computed against the DISCOM's own grid-meter measurements (`BASELINE` and `USAGE` per interval), authored by the utility (BPP) and pushed via `on_status`. The settlement rego ([`demand_flex_revenue.rego`](../../specification/policies/demand_flex_revenue.rego)) is hard-wired to ignore any performance record whose `methodology` is in the vendor-only allowlist (`VENDOR_DEVICE_BACKSTOP`) — even if such a record is the first one in the payload, even if it is the only record on the wire (in which case the rego refuses to settle and surfaces an explicit violation). Baselines do not appear in vendor telemetry at all.

**Vendor telemetry is reconciliation-only.** The seller (aggregator) MAY contribute per-device proof-of-performance telemetry — `USAGE` / `POWER` / `SOC_END` per interval and `GPS_LAT` / `GPS_LON` once per event — which the DISCOM aggregates out-of-band from the aggregator's vendor-API integrations and republishes on the same `on_status` channel as a separate performance record (`methodology: "VENDOR_DEVICE_BACKSTOP"`, status `REPORT_DELIVERED`). It exists so anomalous meter readings can be cross-checked against vendor truth after the fact; it never feeds the revenue flow.
...
Settlement rego is referenced per-contract via `contractAttributes.policy.url` → [`specification/policies/demand_flex_revenue.rego`](../../specification/policies/demand_flex_revenue.rego), package `deg.contracts.demand_flex`. It picks the first `performance[*]` record whose `performanceAttributes.methodology` is NOT in the vendor-only allowlist (`{"VENDOR_DEVICE_BACKSTOP"}`) and computes a net-zero `buyer pays / seller receives` revenue flow from its per-meter `BASELINE` / `USAGE`. If a payload carries only vendor-backstop perf records, the rego emits an explicit `"no settlement-eligible performance record found"` violation rather than coercing vendor numbers into the settlement — see [`demand_flex_revenue_test.rego`](../../specification/policies/demand_flex_revenue_test.rego) `test_vendor_perf_record_excluded_from_settlement` and `test_vendor_only_payload_violation`.
